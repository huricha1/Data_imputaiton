...
class Generator_x(nn.Module):
    def __init__(self):
        ...
        self.time_ser_emd = nn.LSTM(input_size=291, hidden_size=30, num_layers=1, batch_first=True)
        # spatial embeddings
        self.node_emb = nn.Parameter(torch.empty(291, 30))
        nn.init.xavier_uniform_(self.node_emb)
        # temporal embeddings
        self.time_emb = nn.Parameter(torch.empty(30, 30))
        nn.init.xavier_uniform_(self.time_emb)
        self.encode=nn.LSTM(input_size=351, hidden_size=512, num_layers=2, batch_first=True)
    ...
    def forward(self, x,up):
        ...
        if(up==1):
            node_emb=self.node_emb.unsqueeze(0).expand(32,-1,-1)
            time_emb=self.time_emb.unsqueeze(0).expand(32, -1, -1)
        else:
...
class Encoder_x(nn.Module):
    ...
    def __init__(self):
        ...
        self.lstm = nn.LSTM(input_size=291, hidden_size=512, num_layers=2, batch_first=True)
        self.main = nn.Sequential(
            nn.Linear(512, 256),
            nn.BatchNorm1d(30, eps=0.001, momentum=0.01),
            nn.LeakyReLU(0.2, inplace=True),
...
class Discriminator_x(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(
            nn.LSTM(input_size=291, hidden_size=128, num_layers=1, batch_first=True)
        )
...
class Discriminator_i(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(
            nn.LSTM(input_size=291, hidden_size=512, num_layers=2, batch_first=True)
        )
