...
class Generator_x(nn.Module):
    def __init__(self):
        ...
        self.time_ser_emd = nn.LSTM(input_size=6, hidden_size=30, num_layers=1, batch_first=True)
        # spatial embeddings
        self.node_emb = nn.Parameter(torch.empty(6, 30))
        nn.init.xavier_uniform_(self.node_emb)
        # temporal embeddings
        self.time_emb = nn.Parameter(torch.empty(30, 30))
        nn.init.xavier_uniform_(self.time_emb)
        self.encode=nn.LSTM(input_size=66, hidden_size=128, num_layers=2, batch_first=True)
...
class Encoder_x(nn.Module):
    ...
    def __init__(self):
        ...
        self.lstm = nn.LSTM(input_size=6, hidden_size=100, num_layers=2, batch_first=True)
        self.main = nn.Sequential(
...
class Discriminator_i(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(
            nn.LSTM(input_size=6, hidden_size=100, num_layers=2, batch_first=True)
        )
